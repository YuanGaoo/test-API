...
public class RestAPITesting {
	...
	public void simpleGetRequest() {
		...
		given().relaxedHTTPSValidation().when().log().all()
				...
				.get("/posts").then().log().all().assertThat().statusCode(200).and().body("id", hasItem(24))
...
//			.body("sticky", is(false))
	...
	public void simpleGetRequestForSingleItem() {
		...
		given().relaxedHTTPSValidation().when().log().all()
				// .queryParam("per_page",2)
				.pathParam("value", 24).get("/posts/{value}")
				// .get("/posts/{whatever}" , 24)
				// .get("/posts/24) // this is what we want to get eventaully

				.then().log().all().assertThat().statusCode(200).and().body("id", equalTo(24))
				.body("title.rendered", is("Vadym title")).body("sticky", is(false))
	...
	public void simplepPutTest() {
		...
		given().relaxedHTTPSValidation().auth().preemptive().basic("admin", "admin").contentType(ContentType.JSON).
				...
				when().body("{\n" + "	\"title\":\"YY -- API DAY 3 POST time for PUT request\" " + "}").log().all()
				.pathParam("newID", 29).put("/posts/{newID}").then().log().all().statusCode(200)
	...
	public void simpleDeleteTest() {
		...
		given().relaxedHTTPSValidation().auth().preemptive().basic("admin", "admin").when().pathParam("deleteID", 43)
				.queryParam("force", true).delete("/posts/{deleteID}").
				// .delete("/posts/29")
				then().statusCode(200).body("deleted", is(true));
