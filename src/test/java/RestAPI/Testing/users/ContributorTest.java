...
public class ContributorTest {
	 ...
	 * 
	 * 
	 * Scenario : Contributor should be able to submit post for review ---> status
	 * can only be pending
	 * 
	 * 
	 ...
	 * 
	 * 
	 * Scenario : admin user should be able to publish the pending posts by
	 * contributor
	 * 
	 * 
	 */
	...
	public void contributorShouldBeAbleTo_SubmitAPostForAdminReview_Test() {
		...
		given().relaxedHTTPSValidation().auth().preemptive().basic("user1", "user1").contentType(ContentType.JSON)
				.body("{\n" + "	\n" + "	\"title\":\"abc\",\n" + "	\"content\":\"nmy super\",\n"
						+ "	\"status\":\"pending\"\n" + "\n" + "}")
	...
	public void contributorShouldNoyBeAbleTo_PublishAPost_Test() {
		...
		given().relaxedHTTPSValidation().auth().preemptive().basic("user1", "user1").contentType(ContentType.JSON)
				...
				// .statusCode(HttpStatus.SC_CREATED)
				.statusCode(403).header("Content-Type", containsString("application/json"))
	...
	public void adminUser_ShouldBeAbleTo_PublishThePendingPosts() {
		...
		given().relaxedHTTPSValidation().auth().preemptive().basic("admin", "admin").contentType(ContentType.JSON)
				.body("{\"status\":\"publish\"}").pathParam("idOfPendingPost", 56).when()
				.put("/posts/{idOfPendingPost}").then().log().all()
